...
def process_Address(limit_records):

	salesorderlist=[]
	salesorderlist = frappe.get_all('sync Process', filters=[
						["sync Process","reason","=","ADDRESS"],
						], fields=['*'],page_length=int(limit_records))

	if len(salesorderlist)>0:
		for index, order in enumerate(salesorderlist):
			jobname = "Address Check: {}".format(order['order_no'])
			frappe.enqueue(AddressCheck,queue='long',job_name=jobname,timeout=100000,addressTitle=order['order_no'])
			deleteQuery = "DELETE FROM `tabsync Process` WHERE `order_no`='{}'".format(str(order['order_no']))
			test = frappe.db.sql(deleteQuery)

		frappe.db.commit()
	return len(salesorderlist)



@frappe.whitelist(allow_guest=True)
def AddressCheck(addressTitle):

	reply = {}
	reply['message']='Address detail'
	reply['status_code']='200'
	reply['data']=[]


	if not frappe.db.exists('Address', addressTitle):

		url="http://130.211.193.136/api/method/erpnext.flutter_wallet.getAddressDetail?addressTitle={}".format(addressTitle)
		smilyResponse = requests.post(url)
		inventoryUpdated=smilyResponse.json()
		
		oldAddress = inventoryUpdated['message']

		allkeys = oldAddress.keys()
		if len(allkeys)!=0:
			links=oldAddress['links']
			if len(links)!=0:
				customerphone = links[0]['link_name']
				if customerphone not in ['',None]:
					# customerdetail = frappe.db.get("Customer", {"name": customerphone})
					if frappe.db.exists('Customer', customerphone):
						state = findKeys('state',allkeys,oldAddress)
						if state in ['',None," "]:
							state = "Gujarat"

						if state in ["uttarpradesh","Gujrat","Gujrat ","karanataka ","gujarta","gujrat",None,"Gujarat ","west Bengal","AHMEDABAD","gujarat","GUJARAT","gujarat","gujarat ","Gujarat ","GUJARAT "]:
							state = "Gujarat"


						Add_title = findKeys('address_title',allkeys,oldAddress)
						if Add_title in ['',None," "]:
							Add_title = customerphone

						pincode = findKeys('pincode',allkeys,oldAddress)
						if pincode in ['',None," "]:
							pincode = ""

						if len(pincode)!=0:
							if pincode[0] in [0,"0",'',None,' ']:
								pincode=""
							else:
								if len(pincode)!=6:
									pincode = ""
						...
						d1=frappe.get_doc({
							...
							"__unsaved": 1,
							"pincode": pincode,
							"address_line1": oldAddress['address_line1'],
							"address_line2": findKeys('address_line2',allkeys,oldAddress),
							"city": oldAddress['city'],
							"address_title": Add_title,
							"custom_area": findKeys('area',allkeys,oldAddress),
							"is_primary_address": oldAddress['is_primary_address'],
							"state": state,
							"address_type": oldAddress['address_type'],
							"country": oldAddress['country'],
							"is_shipping_address": oldAddress['is_shipping_address'],
							"custom_hideaddress": oldAddress['hideaddress'],
						})
						...
						# appErrorLog("Address Created",addressTitle)

		# reply['respomse']=inventoryUpdated['message']
		reply['respomse']=inventoryUpdated
...
@frappe.whitelist(allow_guest=True)
def process_customer(limit_records):

	salesorderlist=[]
	salesorderlist = frappe.get_all('sync Process', filters=[
						["sync Process","reason","=","CREATECUSTOMER"],
						], fields=['*'],page_length=int(limit_records))

	if len(salesorderlist)>0:
		for index, order in enumerate(salesorderlist):
			jobname = "Check customer orders".format()
			frappe.enqueue(reconsilation_user_detail,queue='long',job_name=jobname,timeout=100000,phone=order['order_no'])
			deleteQuery = "DELETE FROM `tabsync Process` WHERE `order_no`='{}'".format(str(order['order_no']))
			test = frappe.db.sql(deleteQuery)
...
def getTodaysOrders(allow_guest=True):
	return getOrderListForDate(nowdate())
