...
def execute(filters=None):
	...
	# if filters["party"]:
	# 	filters.party = frappe.parse_json(filters["party"])
...
def validate_filters(filters, account_details):
	...
	if filters["account"]:
		if not account_details.get(filters["account"]):
...
def set_account_currency(filters):
	...
	if filters["account"]:
		if len(filters["account"]) == 1:
			...
			currency = get_account_currency(filters["account"])
			is_same_account_currency = True
			if get_account_currency(filters["account"]) != currency:
	...
	elif filters["party"] and filters["party_type"]:
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters["group_by"] == "Group by Account":
...
def get_conditions(filters):
	...
	if filters["voucher_no_not_in"]:
		conditions.append("voucher_no not in %(voucher_no_not_in)s")

	if filters["group_by"] == "Group by Party" and not filters["party_type"]:
	...
	if not (
		filters["account"]
		or filters["party"]
		or filters["group_by"] in ["Group by Account", "Group by Party"]
	):
	...
	if filters["include_default_book_entries"]:
		if filters["finance_book"]:
			if filters["company_fb"] and cstr(filters["finance_book"]) != cstr(
				filters["company_fb"]
			):
				frappe.throw(
	...
	if not filters["show_cancelled_entries"]:
...
def get_data_with_opening_closing(filters, account_details, accounting_dimensions, gl_entries):
	...
	if filters["group_by"] != "Group by Voucher (Consolidated)":
...
def initialize_gle_map(gl_entries, filters, totals_dict):
	gle_map = OrderedDict()
	group_by = group_by_field(filters["group_by"])
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, totals):
	...
	group_by = group_by_field(filters["group_by"])
	group_by_voucher_consolidated = filters["group_by"] == "Group by Voucher (Consolidated)"

	if filters["show_net_values_in_party_account"]:
		account_type_map = get_account_type_map(filters["company"])
	...
	def update_value_in_dict(data, key, gle):
		...
		if filters["add_values_in_transaction_currency"] and key not in ["opening", "closing", "total"]:
		...
		if filters["show_net_values_in_party_account"] and account_type_map.get(data[key].account) in (
			"Receivable",
			"Payable",
		):
	...
	show_opening_entries = filters["show_opening_entries"]
