...
def set_account_currency(filters):
	if filters.get("account") or (filters.get("party") and len(filters.get("party")) == 1):
		filters["company_currency"] = frappe.get_cached_value("Company", filters.get("company"), "default_currency")
		...
		if filters.get("account"):
			if len(filters.get("account")) == 1:
				...
				currency = get_account_currency(filters.get("account"))
			...
			gle_currency = frappe.db.get_value(
				"GL Entry",
				{"party_type": filters.get("party_type"), "party": filters.get("party"), "company": filters.get("company")},
				"account_currency",
			)
			...
			if gle_currency:
				...
				account_currency = (
					None
					if filters.get("party_type") in ["Employee", "Shareholder", "Member"]
					else frappe.get_cached_value(filters.get("party_type"), filters.get("party"), "default_currency")
				)

		filters['account_currency'] = account_currency or filters.get("company_currency")
		if filters.get("account_currency") != filters.get("company_currency") and not filters.get("presentation_currency"):
			filters["presentation_currency"] = filters.get("account_currency")
